...
export default function NftIndex() {
    ...
    async function fetchTrait() {
        try {
            const { data } = await traitService.list({
                search,
                page,
                size,
                status,
    ...
    async function handleDelete(id) {
        try {
            await traitService.remove(id);
            await fetchTrait();
            toast.success("Deleted trait");
        } catch (e) {
            console.log(e);
            toast.success("Cannot Delete trait");
        }
    }
    ...
    async function mint(item) {
        ...
        if (tokentId) {
            await traitService.minted(item.id, tokentId);
            fetchTrait();
        }
    }
    ...
    return (
        <div>
            <TraitView id={viewId} onClose={() => setViewId('')} />

            {confirm}
            <Link to="/admin/trait/create">
                <Button color="info">Create new Trait</Button>
            </Link>
        ...
        <div className="actions-right">
            ...
            { (cell.row.original.status != 'minted') && (
                <>
                    ...
                    <Link to={"/admin/trait/edit/" + cell.row.original._id}>
